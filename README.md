...
- **A real bug was caught and fixed during this live integration.** `_evictable_blocks` was over-pinning prompt-decoded `DOCUMENT` blocks under `pin_generated`, which silently zeroed evictions on the server path used by external harnesses. Root-cause traced via two reproducer scripts (`scripts/repro_opencode_pattern.py`, `scripts/repro_session_pattern.py`); fix in one targeted condition; 106 unit tests still pass.
